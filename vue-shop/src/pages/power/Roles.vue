<template>
    <div>
        ...
        <el-card>
            ...
            <el-row>
                <el-col>
                    <el-button type='primary' @click="addDialog = true">添加角色
                    </el-button>
                </el-col>
            </el-row>
            <!-- 角色列表 -->
            <el-table :data="roleLists" border stripe>
                ...
                <el-table-column label="操作">
                    <template slot-scope='scope'>
                        <el-button size="mini" type='primary' icon="el-icon-search" @click="showEditDialog(scope.row)">
                            编辑
                        </el-button>
        ...
        <!-- 添加角色对话框 -->
        <el-dialog title="添加角色" :visible.sync="addDialog" width="50%" @click='addDialogClosed1'>
            <!-- 内容主题 -->
        ...
        <!-- 修改角色对话框 -->
        <el-dialog title="修改角色" :visible.sync="editDialog" width="50%" @close="editDialogClosed1">
            <!-- 内容主题 -->
        ...
        <el-dialog title="分配权限" :visible.sync="setRightDialogVisible" width="50%" @close="setRightDialogClosed">
            <!-- 树形控件 default-expand-all是否展示所有节点 node-key是通过唯一标识来绑定到勾选框里对应的数据-->
    ...
    export default {
        ...
        methods: {
            ...
            async showEditDialog(ids) {
                // console.log(id)
                const { data: res } = await this.$http.get('roles/' + ids.id)
                if (res.meta.status !== 200) {
            ...
            async removeRightById(role, rightId) {
                ...
                this.$message.success('删除角色成功！')
                // 重新赋予权限 主要是为了防止像this.getRolesList()一样完全刷新数据后 展开列会回缩
